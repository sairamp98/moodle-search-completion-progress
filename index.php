...
$searchusername = optional_param('searchusername', '', PARAM_RAW);

$course = $DB->get_record('course', array('id' => $courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id);

$url = new moodle_url('/report/cpm_completion/index.php', array('course' => $course->id));
$PAGE->set_url($url);
...
require_capability('report/cpm_completion:view', $context);
...
if (!$completion->has_criteria()) {
    throw new \moodle_exception('nocriteriaset', 'report_cpm_completion', $CFG->wwwroot . '/course/report.php?id=' . $course->id);
}
...
if ($csv) {
    ...
    $pluginname = get_string('pluginname', 'report_cpm_completion');
    report_helper::print_report_selector($pluginname);

    // Handle groups (if enabled)
    groups_print_course_menu($course, $CFG->wwwroot . '/report/cpm_completion/index.php?course=' . $course->id);

    // Search form
    echo '<form method="post" action="' . $url . '">';
    echo '<label for="searchusername">' . get_string('searchuser', 'report_cpm_completion') . ': </label>  ';
    echo '<input type="text" name="searchusername" id="searchusername" value="' . s($searchusername) . '">';
    echo ' ';
...
if (!empty($searchusername)) {
    $where[] = $DB->sql_like('u.lastname', ':searchusername', false, false);
    $where_params['searchusername'] = '%' . $searchusername . '%';
}
...
if (!$grandtotal) {
    echo $OUTPUT->container(get_string('err_nousers', 'report_cpm_completion'), 'errorbox errorboxcontent');
    echo $OUTPUT->footer();
    exit;
}
...
$link = $CFG->wwwroot . '/report/cpm_completion/index.php?course=' . $course->id;
if (strlen($sort)) {
    $link .= '&amp;sort=' . $sort;
}
$link .= '&amp;start=';
...
if (!$csv) {
    ...
    $row[] = get_string('id', 'report_cpm_completion');
    $row[] = get_string('name', 'report_cpm_completion');
    foreach ($extrafields as $field) {
    ...
    foreach ($criteria as $criterion) {
        ...
        if ($criterion->criteriatype == COMPLETION_CRITERIA_TYPE_ACTIVITY) {
            ...
            $row[] = $formattedname . ' - ' . get_string('completiondate', 'report_cpm_completion');
        } else {
...
foreach ($progress as $user) {
    ...
    foreach ($criteria as $criterion) {
        ...
        $describe = get_string('completion-' . $completiontype, 'report_cpm_completion');
        ...
        if ($csv) {
            ...
            if ($allow_marking_criteria === $criterion->id) {
                ...
                print '<a href="' . $toggleurl->out() . '" title="' . s(get_string('clicktomarkusercomplete', 'report_cpm__completion')) . '">' .
...
$csvurl = new moodle_url('/report/cpm_completion/index.php', array('course' => $course->id, 'format' => 'csv'));
$excelurl = new moodle_url('/report/cpm_completion/index.php', array('course' => $course->id, 'format' => 'excelcsv'));
...
$event = \report_cpm_completion\event\report_viewed::create(array('context' => $context));
$event->trigger();
